...
df_data = pd.read_excel('API_19_DS2_en_excel_v2_21245.xls')

df_new = df_data.iloc[2:]

filtered_df = df_new[df_new['Unnamed: 2'].isin(['Urban population','Population, total','Renewable electricity output (% of total electricity output)','Renewable energy consumption (% of total final energy consumption)'])]
...
pivoted[['Urban population','Population, total']] = pivoted[['Urban population','Population, total']].astype(int)
...
plt.figure()
plt.scatter( pivoted["Population, total"],pivoted["Renewable electricity output (% of total electricity output)"], 10, marker="o")
